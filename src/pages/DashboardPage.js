...
function Dashboard() {
  ...
  const [header, setHeader] = useState([
    ...
    "price",
    "availability",
    "photo1", // Make sure 'photo1' and 'photo2' are included in the header
    "photo2",
  ]);
  ...
  const handleData = (type) => {
    if (type == "Users") {
      ...
      axios
        .get(`${process.env.REACT_APP_API_BASE_URL}/cars`)
        .then((response) => {
          setHeader([
            "id",
            "photo1",
      ...
      axios
        .get(`${process.env.REACT_APP_API_BASE_URL}/rents`)
        .then((response) => {
          setHeader([
            "id",
            "rental_date",
  ...
  const handleUpdateItem = (itemId, updatedItem) => {
    const endpoint = `${process.env.REACT_APP_API_BASE_URL}/${type}/${itemId}`;
  ...
  const handleDelete = (id) => {
    const endpoint = `${process.env.REACT_APP_API_BASE_URL}/${type}/${id}`;
                  ...
                  <Table variant="striped" size={{ base: "sm", md: "md" }}>
                    ...
                    <Tbody>
                      {data.map((item) => (
                        <Tr key={item.id}>
                          {header.map((column) => {
                            if (column === "photo1" || column === "photo2") {
                              return (
                                <Td key={`${item.id}-${column}`}>
                                  {item[column] && (
                                    <Image
                                      src={item[column]}
                                      alt={`${column} image`}
                                      boxSize="100px" // Set image size
                                      objectFit="cover" // Adjust fit to maintain aspect ratio
                                    />
                                  )}
                                </Td>
                              );
